...
export default function EditCasaParceiraPage({ params }: { params: Promise<{ id: string; slug: string }> }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (casaEncontrada) {
          ...
          setFormData({
            nome: casaEncontrada.nome || '',
            slug: casaEncontrada.slug || '',
            logo_url: casaEncontrada.logo_url || '',
            link_base: casaEncontrada.link_base || '',
            codigo_afiliado: casaEncontrada.codigo_afiliado || '',
            utm_source: casaEncontrada.utm_source || '',
            utm_medium: casaEncontrada.utm_medium || '',
            utm_campaign: casaEncontrada.utm_campaign || '',
            ativo: casaEncontrada.ativo ?? true
          });

          // Preencher configurações de canal
          if (casaEncontrada.canais_config) {
            setCanaisConfig({
              geral: casaEncontrada.canais_config.geral || { utm_content: '', utm_term: '' },
              instagram: {
                close_friends: casaEncontrada.canais_config.instagram?.close_friends || { utm_content: '', utm_term: '' },
                normal: casaEncontrada.canais_config.instagram?.normal || { utm_content: '', utm_term: '' }
              },
              telegram: {
                free: casaEncontrada.canais_config.telegram?.free || { utm_content: '', utm_term: '' },
                vip: casaEncontrada.canais_config.telegram?.vip || { utm_content: '', utm_term: '' }
              }
            });
          }
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${buildApiUrl(API_CONFIG.ENDPOINTS.PARTNER_HOUSES)}/${casa.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        throw new Error(errorData.detail || 'Erro ao atualizar casa parceira');
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Erro desconhecido');
    } finally {
    ...
    <div className="container mx-auto py-8">
      ...
      {loading ? (
        ...
        <div className="max-w-4xl mx-auto">
          <Card>
            <CardHeader>
              <CardTitle className="text-2xl">Editar Casa Parceira: {casa?.nome}</CardTitle>
            </CardHeader>
            <CardContent>
              {error && (
